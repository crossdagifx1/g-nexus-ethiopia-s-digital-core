...
export const CareersManager = () => {
  ...
  const fetchPositions = async () => {
    ...
    setPositions((data as Position[]) || []); setLoading(false);
  };
  ...
  const openEdit = (p: Position) => { setEditing(p); setForm({ title: p.title, department: p.department, type: p.type, location: p.location || '', description: p.description || '', requirements: JSON.stringify(p.requirements || []), status: p.status }); setDialogOpen(true); };
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      ...
      await logActivity('Updated career position', 'career_position', editing.id, { title: form.title });
    } else {
      const { error } = await supabase.from('career_positions').insert(record);
      if (error) { toast({ title: 'Error', variant: 'destructive' }); return; }
      await logActivity('Created career position', 'career_position', undefined, { title: form.title });
    }
    toast({ title: editing ? 'Position updated' : 'Position added' }); setDialogOpen(false); fetchPositions();
  };

  const handleDelete = async (id: string) => {
    await supabase.from('career_positions').delete().eq('id', id);
    await logActivity('Deleted career position', 'career_position', id);
    toast({ title: 'Deleted' }); fetchPositions();
  };
    ...
    <div>
      ...
      <Table>
        ...
        <TableBody>
          {filtered.map(p => (
            <TableRow key={p.id}>
              <TableCell className="font-medium">{p.title}</TableCell><TableCell>{p.department}</TableCell><TableCell>{p.type}</TableCell>
              <TableCell><Badge variant={p.status === 'open' ? 'default' : 'secondary'}>{p.status}</Badge></TableCell>
              <TableCell><div className="flex gap-1"><Button variant="ghost" size="icon" onClick={() => openEdit(p)}><Edit2 className="w-4 h-4" /></Button><Button variant="ghost" size="icon" className="text-destructive" onClick={() => handleDelete(p.id)}><Trash2 className="w-4 h-4" /></Button></div></TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3"><div><Label>Title</Label><Input value={form.title} onChange={e => setForm({...form, title: e.target.value})} /></div><div><Label>Department</Label><Input value={form.department} onChange={e => setForm({...form, department: e.target.value})} /></div></div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label>Type</Label><Select value={form.type} onValueChange={v => setForm({...form, type: v})}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Full-time">Full-time</SelectItem><SelectItem value="Part-time">Part-time</SelectItem><SelectItem value="Contract">Contract</SelectItem></SelectContent></Select></div>
              <div><Label>Status</Label><Select value={form.status} onValueChange={v => setForm({...form, status: v})}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="open">Open</SelectItem><SelectItem value="closed">Closed</SelectItem></SelectContent></Select></div>
            </div>
