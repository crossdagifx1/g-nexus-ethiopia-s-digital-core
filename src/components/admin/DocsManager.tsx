...
export const DocsManager = () => {
  ...
  const fetchDocs = async () => {
    ...
    setDocs((data as DocEntry[]) || []); setLoading(false);
  };
  useEffect(() => { fetchDocs(); }, []);
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      await supabase.from('documentation_entries').update(record).eq('id', editing.id);
      await logActivity('Updated documentation', 'documentation', editing.id, { title: form.title });
    } else {
      await supabase.from('documentation_entries').insert(record);
      await logActivity('Created documentation', 'documentation', undefined, { title: form.title });
    }
    toast({ title: editing ? 'Doc updated' : 'Doc added' }); setDialogOpen(false); fetchDocs();
  };

  const handleDelete = async (id: string) => {
    await supabase.from('documentation_entries').delete().eq('id', id);
    await logActivity('Deleted documentation', 'documentation', id);
    toast({ title: 'Deleted' }); fetchDocs();
  };
        ...
        <TableBody>
          {filtered.map(d => (
            <TableRow key={d.id}>
              ...
              <TableCell><div className="flex gap-1"><Button variant="ghost" size="icon" onClick={() => openEdit(d)}><Edit2 className="w-4 h-4" /></Button><Button variant="ghost" size="icon" className="text-destructive" onClick={() => handleDelete(d.id)}><Trash2 className="w-4 h-4" /></Button></div></TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3"><div><Label>Category</Label><Input value={form.category} onChange={e => setForm({...form, category: e.target.value})} /></div><div><Label>Icon</Label><Input value={form.icon} onChange={e => setForm({...form, icon: e.target.value})} /></div><div><Label>Order</Label><Input type="number" value={form.display_order} onChange={e => setForm({...form, display_order: parseInt(e.target.value) || 0})} /></div></div>
