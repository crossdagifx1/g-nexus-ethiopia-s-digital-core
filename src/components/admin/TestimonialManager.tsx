...
export const TestimonialManager = () => {
  ...
  const handleSave = async () => {
    const payload: any = { quote: form.quote, author_name: form.author_name, author_role: form.author_role || null, author_company: form.author_company || null, avatar_url: form.avatar_url || null, rating: form.rating, featured: form.featured };
    if (editing) {
      await (supabase.from('testimonials' as any) as any).update(payload).eq('id', editing.id);
      toast({ title: 'Testimonial updated' });
    } else {
      await (supabase.from('testimonials' as any) as any).insert(payload);
  ...
  const toggleFeatured = async (t: Testimonial) => {
    await (supabase.from('testimonials' as any) as any).update({ featured: !t.featured }).eq('id', t.id);
    fetch();
  };
          ...
          <div key={t.id} className={`p-5 rounded-2xl border ${t.featured ? 'border-primary bg-primary/5' : 'border-border bg-card'}`}>
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex gap-1">
                <Button variant="ghost" size="icon" onClick={() => toggleFeatured(t)}>
                  <Star className={`w-4 h-4 ${t.featured ? 'text-yellow-500 fill-yellow-500' : 'text-muted-foreground'}`} />
                </Button>
