...
export const TeamManager = () => {
  ...
  const openEdit = (m: TeamMember) => {
    ...
    setForm({
      ...
      fun_fact: m.fun_fact || '', skills: JSON.stringify(m.skills || []),
      social_links: JSON.stringify(m.social_links || {}), display_order: m.display_order, is_active: m.is_active,
    });
    setDialogOpen(true);
  };
  ...
  return (
    <div>
      ...
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead>Name</TableHead><TableHead>Role</TableHead><TableHead>Active</TableHead><TableHead>Order</TableHead><TableHead>Actions</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {filtered.map(m => (
            <TableRow key={m.id}>
              ...
              <TableCell>
                <div className="flex gap-1">
                  <Button variant="ghost" size="icon" onClick={() => openEdit(m)}><Edit2 className="w-4 h-4" /></Button>
